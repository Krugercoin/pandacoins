...
unsigned int nStakeTargetSpacing = 2*60; //  60 second block spacing
int64 nChainStartTime = 1392380467;
int nCoinbaseMaturity = 1420;
...
const string strMessageMagic = "pandacoins Signed Message:\n";
...
static const int64 nTargetTimespan = 0.1 * 24 * 60 * 60;  // one week
static const int64 nTargetSpacingWorkMax = 10 * nStakeTargetSpacing; // 2-hour
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in PandaCoins
    bool fStrictPayToScriptHash = true; // Always active in PandaCoins
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "pandacoins", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
  ...
  //vMerkleTree: 05b4eb8ba8 
        ...
        // Genesis block
        const char* pszTimestamp = "Panda Coins Have Arrived.";
        CTransaction txNew;
        ...
        block.nNonce   = 1142159 ;

 	   		if (true && (block.GetHash() != hashGenesisBlock)) {

					// This will figure out a valid hash and Nonce if you're
					// creating a different genesis block:
					fprintf(stderr,"Start search a genesis block!\n");
		    	...
		    	uint256 hash = block.GetHash();
		          ...
		          hash = block.GetHash();
		          if((block.nNonce & 0xfff) == 0 || hash <=hashTarget){
		          	fprintf(stderr,"testHash== %s\n", hash.ToString().c_str());
		        	}
		       	}
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xae70507c2ac0ac401f28360e05f9884bca5a5c7f7ea1aa25dbedf9efbd526487"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("PandacoinsMiner : proof-of-work not meeting target");

    //// debug print
    printf("PandacoinsMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PandacoinsMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("PandacoinsMiner : ProcessBlock, block not accepted");
    }
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        {
            if(fDebug) printf("vNodes.size() == %d, IsInitialBlockDownload() == %d\n", vNodes.size(), IsInitialBlockDownload());
            Sleep(1000);
            if (fShutdown)
                return;
        ...
        while (pwallet->IsLocked())
        {
            ...
            if(fDebug) printf("wallet be locked,then sleep 1 second!\n");
            Sleep(1000);
        }
        strMintWarning = "";
        ...
        // Create new block
        //
        if(fDebug) printf("It will create a new block!\n");
        unsigned int nTransactionsUpdatedLast = nTransactionsUpdated;
        CBlockIndex* pindexPrev = pindexBest;

        auto_ptr<CBlock> pblock(CreateNewBlock(pwallet, fProofOfStake));
        if (!pblock.get()){
            if(fDebug) printf("Can't create new block,will be return!\n");
            return;
          }
        if(fDebug) printf("A new block be created!\n");
        IncrementExtraNonce(pblock.get(), pindexPrev, nExtraNonce);
        if (fProofOfStake)
        {
            // ppcoin: if proof-of-stake block found then process block
            if(fDebug) printf("A proof-of-stake block be founded,!\n");
            if (fDebug && !pblock->IsProofOfStake()) printf("A proof-of-stake block can't be confirmmed!\n");
            if (pblock->IsProofOfStake())
            {
                if (!pblock->SignBlock(*pwalletMain))
                {
                    if(fDebug) printf("A proof-of-stake block can't be signed,!\n");
                    strMintWarning = strMintMessage;
                    continue;
                }
                strMintWarning = "";
        ...
        printf("Running PandacoinsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
        ...
        while(true)
        {
            ...
            while(true)
            {
                scrypt_1024_1_1_256_sp_city(BEGIN(pblock->nVersion), BEGIN(thash), scratchpad);
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadPandacoinsMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadPandacoinsMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadPandacoinsMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d PandacoinsMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadBitcoinMiner, pwallet))
                printf("Error: NewThread(ThreadPandacoinsMiner) failed\n");
            Sleep(10);
        }
    }
}
